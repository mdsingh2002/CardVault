...
import collectionHistoryApi, {
  type CollectionValueSnapshot,
} from "@/services/collectionHistoryApi";
...
export default function CollectionValueChart() {
  ...
  const fetchHistory = async (days: number) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || "Failed to load collection history"
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Track your collection's value history
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="h-[350px] flex flex-col items-center justify-center">
          <p className="text-muted-foreground mb-4">
            No historical data available yet. Add cards to your collection to
            start tracking value over time.
          </p>
          <p className="text-sm text-muted-foreground">
            Price snapshots are automatically recorded when you add or remove
            cards.
          </p>
        </CardContent>
      </Card>
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              {valueChange >= 0 ? "+" : ""}
              {percentChange}% change over selected period
            </CardDescription>
          </div>
          ...
          >
            ...
            <Tooltip
              formatter={(value: number, name: string) => {
                if (name === "value")
                  return [`$${value.toFixed(2)}`, "Collection Value"];
